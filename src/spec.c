...
RedisModuleString *IndexSpec_GetFormattedKeyByName(IndexSpec *sp, const char *s,
                                                   FieldType forType) {
  const FieldSpec *fs = IndexSpec_GetFieldC(sp, s, strlen(s));
  if (!fs) {
    return NULL;
  }
...
static void IndexSpec_ScanAndReindexAsync(StrongRef spec_ref) {
  ...
  Obfuscate_Index(spec->uniqueId, name);
  RedisModule_Log(NULL, "notice", "Register index %s for async scan", name);
#endif
...
void IndexSpec_DropLegacyIndexFromKeySpace(IndexSpec *sp) {
  ...
  HiddenName_DropFromKeySpace(ctx.redisCtx, INDEX_SPEC_KEY_FMT, sp->name);
}
...
void *IndexSpec_LegacyRdbLoad(RedisModuleIO *rdb, int encver) {
  ...
  if (!sp->rule) {
    RedisModule_LogIOError(rdb, "warning", "Failed creating rule for legacy index '%s', error='%s'",
                           name, QueryError_GetError(&status));
    StrongRef_Release(spec_ref);
    return NULL;
  }
  ...
  while ((entry = dictNext(iter))) {
    ...
    if (sp->aliases) {
      ...
      for (size_t ii = 0; ii < array_len(sp->aliases); ++ii) {
        HiddenName_SaveToRdb(sp->aliases[ii], rdb);
      }
    } else {
      RedisModule_SaveUnsigned(rdb, 0);
    }
  ...
  if (spec->flags & Index_HasVecSim) {
    for (int i = 0; i < spec->numFields; ++i) {
      if (spec->fields[i].types == INDEXFLD_T_VECTOR) {
        RedisModuleString *rmskey = HiddenString_CreateString(spec->fields[i].name, ctx);
